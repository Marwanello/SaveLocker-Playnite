...
namespace SaveLocker.Playnite
{
    ...
    internal sealed class LocalApiClient
    {
        ...
        private async Task<T> SendAsync<T>(HttpMethod method, string route, object body, Func<object, T> parse, CancellationToken ct)
        {
        ...
        {
            return await SendAsync(HttpMethod.Post, $"/api/games/{gameId}/pre-launch-sync", new { }, o => LaunchGateResult.FromJson(Json.AsObject(o)), ct).ConfigureAwait(false);
        }

        public async Task PostExitSyncAsync(Guid gameId, CancellationToken ct = default(CancellationToken))
        {
            await SendAsync<object>(HttpMethod.Post, $"/api/games/{gameId}/post-exit-sync", new { }, o => null, ct).ConfigureAwait(false);
        }
